  ...
  return (
    <div className="bg-white text-[#333333] rounded-lg p-8 w-full mx-auto space-y-8 border border-[#4ECFBF]/30 shadow-md">
      ...
      {status === 'complete' && assessment && (
        <div>
          {/* Guest User Simplified View */}
          {!isAuthenticated() ? (
            <div className="grid grid-cols-1 gap-6 max-w-3xl mx-auto">
              {/* Playback Controls */}
              <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
                <h3 className="text-lg font-medium text-[#333333] mb-4">Your Recording</h3>
                <div className="flex items-center justify-center space-x-4">
                  <Button
                    onClick={handlePlayAudio}
                    disabled={!audioUrl}
                    className={`w-12 h-12 rounded-full flex items-center justify-center ${isAudioPlaying ? 'bg-red-500 hover:bg-red-600' : 'bg-[#4ECFBF] hover:bg-[#3DBFAF]'} text-white shadow-md transition-all duration-300`}
                  >
                    {isAudioPlaying ? <Square className="h-5 w-5" /> : <Play className="h-5 w-5 ml-0.5" />}
                  </Button>
                  <audio ref={audioPlayerRef} src={audioUrl || ''} onEnded={() => setIsAudioPlaying(false)} />
                </div>
              </div>
              ...
              <div className="bg-[#FFD63A]/20 border-l-4 border-[#FFD63A] p-4 rounded-r-lg">
                <div className="flex items-start">
                  <div className="flex-shrink-0">
                    <AlertCircle className="h-5 w-5 text-[#FFD63A]" />
                  </div>
                  <div className="ml-3">
                    <h3 className="text-sm font-medium text-[#333333]">Want more detailed analysis?</h3>
                    <div className="mt-1 text-sm text-[#555555]">
                      <p>{getGuestLimitationsDescription()}</p>
                    </div>
                    <div className="mt-3">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-5xl mx-auto">
                ...
                <div className="space-y-6">
                  ...
                  {/* Recommended Level - Shown to both guest and registered users */}
                  <div className="bg-[#F0FDFB] p-6 rounded-lg text-center border border-[#4ECFBF] shadow-md">
                    <h3 className="text-xl text-[#333333] mb-3 font-medium">Recommended Level</h3>
                    <div className="text-5xl font-bold text-[#333333] mb-3">{assessment.recommended_level}</div>
                    <div className="inline-block bg-[#4ECFBF] px-4 py-2 rounded-full text-white text-sm font-medium shadow-md">
