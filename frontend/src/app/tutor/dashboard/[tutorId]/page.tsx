...
export default function TutorDashboardPage() {
  ...
  const loadDashboardData = async (token: string, filters = {}) => {
    try {
      ...
      const url = `${getApiBaseUrl()}/tutor/dashboard/${tutorId}/learners?${queryParams.toString()}`;
  ...
  const loadAnalytics = async (token: string) => {
    try {
      const response = await fetch(`${getApiBaseUrl()}/tutor/dashboard/${tutorId}/analytics`, {
        headers: {
          'Authorization': `Bearer ${token}`,
        },
      });
  ...
  const handleViewDetails = async (learner: Learner) => {
    ...
    try {
      ...
      const response = await fetch(
        `${getApiBaseUrl()}/tutor/dashboard/${tutorId}/learner/${learner.user_id}/details`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
          },
